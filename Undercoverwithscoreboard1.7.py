#known bugs : 
#points might not update for eliminated winning team members
#balance is meh right now
...
def update_scoreboard (list_of_players, list_of_roles, role = ""):
    ...
    if role == "Civilian" :
      for player in list_of_roles :
        scoreboard.update({list_of_players[list_of_roles.index(player)] : +2 })
       
    if role == "Imposter" :
      for player in list_of_roles :
        scoreboard.update({list_of_players[list_of_roles.index(player)] : +3 })
        
    if role == "Mr.White" :
      scoreboard.update({list_of_players[list_of_roles.index("Mr White")] : +4 })
