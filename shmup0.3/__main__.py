...
def draw_window(player_ship, background, background2, player_lives, player_won, current_boss, boss_is_hurt, *_):
    ...
    if player_won == True :
        player_win_display = HEALTHDISP_FONT.render("YOU WON ! GOOD JOB !", 1, WHITE)
        WIN.blit(player_win_display, (WIDTH/2 - player_win_display.get_width()/2, HEIGHT/2-200))
    if boss_is_hurt == False :
...
def player_movement(clock, player_ship, keys_pressed):
    ...
    #MOVEMENT
    if keys_pressed[pygame.K_UP] and (player_ship.position[1]  - player_ship.velocity) > 0 :
...
def handle_bullets (player_ship, current_boss):
    def handle_player_bullets():
        for bullet in PLAYER_BULLETS :
            ...
            if current_boss.hitbox().colliderect(bullet.hitbox()):
...
LIST_OF_BOSSES =  deque([None, bosstest, boss1]) #MUST START WITH NONE
