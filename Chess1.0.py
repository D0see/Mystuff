...
def the_game():
  ...
  def define_row_length():
    ...
    while row_length <= 3 or row_length > 8 or row_length == 5 :
      row_length = int(input("how many cases per row ?" + "\n"))
    return row_length#define row_length
  
  #print the board 
  def print_grid(grid, row_length):
      grid_to_print = [*grid.items()]
      for i in range(row_length):
        print(grid_to_print[(0+i)*row_length:(1+i)*row_length])#print a grid in a more readable way
  ...
  def make_and_populate_a_grid(row_length):
    ...
    def populate_grid(grid, row_length):
      ...
      def populate_grid_with_bishops(grid, row_length):
        if row_length % 2 == 0 :
          for coordinates in grid :
            if coordinates[0] == 1 and coordinates[1] == row_length / 2 - 1 :
              grid.update({coordinates : "BBishop1"})
            if coordinates[0] == 1 and coordinates[1] == row_length / 2 + 2 :
              grid.update({coordinates : "BBishop2"})
            if coordinates[0] == row_length and coordinates[1] == row_length / 2 - 1 :
              grid.update({coordinates : "WBishop1"})
            if coordinates[0] == row_length and coordinates[1] == row_length / 2 + 2 :
              grid.update({coordinates : "WBishop2"})  
        else :
          for coordinates in grid :
            if coordinates[0] == 1 and coordinates[1] == round(row_length/ 2 ) - 1 :
              grid.update({coordinates : "BBishop1"})
            if coordinates[0] == 1 and coordinates[1] == round(row_length/ 2 ) + 1 :
              grid.update({coordinates : "BBishop2"})  
            if coordinates[0] == row_length and coordinates[1] == round(row_length/ 2 ) - 1 :
              grid.update({coordinates : "WBishop1"})
            if coordinates[0] == row_length and coordinates[1] == round(row_length/ 2 ) + 1 :
              grid.update({coordinates : "WBishop2"})  
      populate_grid_with_bishops(grid, row_length)      
  
      def populate_grid_with_knights(grid, row_length):
        if row_length % 2 == 0 :
          for coordinates in grid :
            if coordinates[0] == 1 and coordinates[1] == row_length / 2 - 2 :
              grid.update({coordinates : "BKnight1"})
            if coordinates[0] == 1 and coordinates[1] == row_length / 2 + 3 :
              grid.update({coordinates : "BKnight2"})
            if coordinates[0] == row_length and coordinates[1] == row_length / 2 - 2 :
              grid.update({coordinates : "WKnight1"})
            if coordinates[0] == row_length and coordinates[1] == row_length / 2 + 3 :
              grid.update({coordinates : "WKnight2"})  
        else :
          for coordinates in grid :
            if coordinates[0] == 1 and coordinates[1] == round(row_length/ 2 ) - 2 :
              grid.update({coordinates : "BKnight1"})
            if coordinates[0] == 1 and coordinates[1] == round(row_length/ 2 ) + 2 :
              grid.update({coordinates : "BKnight2"})  
            if coordinates[0] == row_length and coordinates[1] == round(row_length/ 2 ) - 2 :
              grid.update({coordinates : "WKnight1"})
            if coordinates[0] == row_length and coordinates[1] == round(row_length/ 2 ) + 2 :
              grid.update({coordinates : "WKnight2"})  
      populate_grid_with_knights(grid, row_length)    
      
      def populate_grid_with_rooks(grid, row_length):
        if row_length % 2 == 0 :
          for coordinates in grid :
            if coordinates[0] == 1 and coordinates[1] == row_length / 2 - 3 :
              grid.update({coordinates : "BRook1"})
            if coordinates[0] == 1 and coordinates[1] == row_length / 2 + 4 :
              grid.update({coordinates : "BRook2"})
            if coordinates[0] == row_length and coordinates[1] == row_length / 2 - 3 :
              grid.update({coordinates : "WRook1"})
            if coordinates[0] == row_length and coordinates[1] == row_length / 2 + 4 :
              grid.update({coordinates : "WRook2"})  
        else :
          for coordinates in grid :
            if coordinates[0] == 1 and coordinates[1] == round(row_length/ 2 ) - 3 :
              grid.update({coordinates : "BRook1"})
            if coordinates[0] == 1 and coordinates[1] == round(row_length/ 2 ) + 3 :
              grid.update({coordinates : "BRook2"})  
            if coordinates[0] == row_length and coordinates[1] == round(row_length/ 2 ) - 3 :
              grid.update({coordinates : "WRook1"})
            if coordinates[0] == row_length and coordinates[1] == round(row_length/ 2 ) + 3 :
              grid.update({coordinates : "WRook2"})       
      populate_grid_with_rooks(grid, row_length)     
  ...
  def game_logic(grid) :
      ...
      def king_safety_check(grid, turn, anti_turn):
        ...
        for coordinates, piece_ in grid.items():
          ...
          if piece == "Queen":
            ...
            for threat in list_of_threat_temp :
              print(f"{anti_turn}queen can go there",threat)
              list_of_threat_to_king.append(threat)
      ...
      def check_for_pat(grid, turn, anti_turn):
      ...
      def move(turn, grid): 
       ...
       #handles player input like : "WPawn1" , also handles Resign and Score.
        def player_entry(turn):
          ...
          while turn + piece not in list_of_elements_on_board :
            ...
            if piece == "Resign":
              ...
              print("Available commands are : FirstletterofcolorPiecenamePiecenumber, Resign, Advantage, Help")
          return turn + piece
        ...
        def which_piece(player_input):
          ...
          def has_legal_moves(list_of_possible_moves, piece_location):    
              if list_of_possible_moves == []:
                print("This piece has no available moves")
                move(turn, grid)
              else :
                where_to(list_of_possible_moves, piece_location)    
          #3present the player with a selection of possible moves for the selected piece, then delete the piece at old position and create the piece at the selected position (contains pawn promotion logic)
          ...
          if piece == "Pawn" :
            has_legal_moves(move_pawn(turn, piece_location), piece_location)
              
          #1makes list of possible move if the selected piece is a Rook
          elif piece == "Rook" :
            has_legal_moves(move_rook(turn, anti_turn, piece_location), piece_location)   
            
          #1makes list of possible move if the selected piece is a Bishop
          elif piece == "Bishop" :
            has_legal_moves(move_bishop(turn, anti_turn, piece_location), piece_location)    
                     
          #1makes list of possible move if the selected piece is a Knight
          elif piece == "Knight" :
            has_legal_moves(move_knight(turn, piece_location), piece_location)
                            
          #1makes list of possible move if the selected piece is a Queen
          elif piece == "Queen" :
            has_legal_moves(move_queen(turn, anti_turn, piece_location), piece_location)     
      
          #1makes list of possible move if the selected piece is a King  
          elif piece == "King" :
              has_legal_moves(move_king(turn, piece_location), piece_location)
      ...
      def function_sequence(playground):
          while True == True :
              if king_safety_check(playground, turn, anti_turn) == False :
                if check_for_pat(playground, turn, anti_turn) == True :
                  if turn == "W":
                    print("\n" + "Black won !" + "\n" )
                    the_game()
                  if turn == "B":
